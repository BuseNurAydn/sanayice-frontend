...
const FilterSection = ({ title, children }) => (
  <div className="border-b border-gray-200 py-3">
    ...
    <div className="mt-2 text-sm max-h-60 overflow-y-auto space-y-1">{children}</div>
  </div>
);
...
const BannerPage = () => {
  ...
  useEffect(() => {
    ...
    switch (sortOption) {
      ...
      case "highPrice": filtered.sort((a, b) => b.price - a.price); break;
      case "lowPrice": filtered.sort((a, b) => a.price - b.price); break;
      default: break;
    }
    setProducts(filtered);
  }, [filters, sortOption, allProducts]);

  if (!category) return <h1 className="text-4xl p-10 text-center font-bold text-red-700">404 - Kategori bulunamadı</h1>;
  if (error && !loading) return (
    <div className="container mx-auto p-10 text-center">
      <h1 className="text-3xl font-bold text-red-600 mb-4">{category.name}</h1>
          ...
          <div className="sticky top-4 bg-white p-4 rounded-xl shadow-lg border border-gray-100">
              ...
              {brands.slice(0, showAllBrands ? brands.length : 5).map(({ brand, count }) => (
                <label key={brand} className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox"
                    ...
                    onChange={(e) => {
                      if (e.target.checked) setFilters(prev => ({ ...prev, brands: [...prev.brands, brand] }));
                      else setFilters(prev => ({ ...prev, brands: prev.brands.filter(x => x !== brand) }));
                    }}
                  />
                  <span className="text-gray-700">{brand} <span className="text-gray-400">({count})</span></span>
                </label>
              ))}
              {brands.length > 5 && (
                <button onClick={() => setShowAllBrands(prev => !prev)} className="text-orange-500 text-sm mt-2">
            ...
            <FilterSection title="Fiyat Aralığı">
              <div className="flex items-center gap-2 mb-2">
                <input type="number" value={filters.minPrice} min={priceRange.min} max={filters.maxPrice || priceRange.max}
                  onChange={e => setFilters(prev => ({ ...prev, minPrice: e.target.value }))}
                  className="w-1/2 border border-gray-300 rounded px-2 py-1 text-sm" placeholder="Min" />
                <input type="number" value={filters.maxPrice} min={filters.minPrice || priceRange.min} max={priceRange.max}
                  onChange={e => setFilters(prev => ({ ...prev, maxPrice: e.target.value }))}
                  className="w-1/2 border border-gray-300 rounded px-2 py-1 text-sm" placeholder="Max" />
              </div>
              <input type="range" min={priceRange.min} max={priceRange.max} step={100} value={filters.maxPrice || priceRange.max}
                onChange={e => setFilters(prev => ({ ...prev, maxPrice: e.target.value }))} className="w-full" />
              <p className="text-sm mt-1">{filters.minPrice} TL - {filters.maxPrice} TL</p>
            </FilterSection>

            <button onClick={() => setFilters({ brands: [], minPrice: priceRange.min, maxPrice: priceRange.max })}
        ...
        <main className="lg:col-span-9">
          <div className="flex justify-between items-center mb-6 p-4 bg-white rounded-xl shadow-md border-t-4 border-orange-500">
            <p className="text-md text-gray-700">
              {loading ? "Ürünler yükleniyor..." : `Toplam ${products.length} ürün listeleniyor.`}
            </p>
            <select className="border border-gray-300 rounded-lg p-2 text-sm focus:ring-orange-500 focus:border-orange-500"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {loading
              ...
              : products.length
                ...
                : <p className="col-span-full text-center text-xl text-gray-500 py-10">Bu kategoride ürün bulunmamaktadır.</p>
            }
          </div>
        </main>
      </div>
