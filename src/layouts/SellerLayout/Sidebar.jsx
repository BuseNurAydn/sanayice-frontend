...
const Sidebar = () => {
  ...
  const filteredMenuItems = menuItems.filter((item) => {
    if (role === 'ROLE_SELLER') {
      ...
      return item.label !== 'Kategoriler' && item.label !== 'Marka Listesi' && item.label !== 'Satıcı Doğrulama' && item.label !== 'Ürün Onaylama' && item.label !== 'Slider Yönetimi' && item.label !== 'Kampanya Ve Kupon Yönetimi' && item.label !== 'Satıcıların Kupon Ve Kampanya Listesi' && item.label !== 'Destek Ve Geri Bildirim Yönetimi' && item.label !== 'Satıcı Doğrulama Sayfası'
    } else if (role === 'ROLE_MANAGER') {
      return item.label === 'Satıcı Doğrulama' || item.label === 'Ürün Onaylama' || item.label === 'Kategoriler' || item.label === 'Marka Listesi' || item.label === 'Slider Yönetimi' || item.label === 'Kampanya Ve Kupon Yönetimi' || item.label === 'Satıcıların Kupon Ve Kampanya Listesi' || item.label === 'Destek Ve Geri Bildirim Yönetimi' || item.label === 'Satıcı Doğrulama Sayfası'
    } else {
      return true
    }
  })
  ...
  return (
    <>
      {/* Mobile Overlay */}
      {isMobileOpen && <div className='fixed inset-0 bg-opacity-50 z-30 md:hidden' onClick={() => setIsMobileOpen(false)} />}

      {/* Mobile Menu Button */}
      <button className='md:hidden fixed top-2 left-2 z-50 p-2 bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-200' onClick={() => setIsMobileOpen(true)}>
        <GiHamburgerMenu className='w-5 h-5 text-gray-700' />
      </button>

      {/* Sidebar */}
      <aside
        className={`${collapsed ? 'w-20' : 'w-64'} bg-gradient-to-b from-slate-50 to-slate-100 border-r border-slate-200 min-h-screen 
      ...
      >
        ...
        <div className='flex items-center justify-between p-4 border-b border-slate-200 bg-white/50'>
          {!collapsed && <h2 className='text-lg pl-12 font-semibold text-slate-800 truncate'>  {role === 'ROLE_SELLER' ? 'Satıcı Paneli' : 'Yönetici Paneli'}</h2>}
          ...
          >
            {collapsed ? <BsChevronDoubleRight className='w-4 h-4' /> : <BsChevronDoubleLeft className='w-4 h-4' />}
          </button>
        ...
        <nav className='p-3 space-y-1'>
          {filteredMenuItems.map(({ label, to, icon, subMenu }, index) => {
            ...
            return (
              <div key={index} className='space-y-1'>
                ...
                >
                  <div className='flex-shrink-0 text-slate-500 group-hover:text-blue-600 transition-colors duration-200'>{icon}</div>

                  {!collapsed && (
                    <>
                      <span className='flex-1 font-medium text-sm truncate'>{label}</span>
                      {hasSubMenu && <div className='flex-shrink-0 text-slate-400 group-hover:text-blue-500 transition-colors duration-200'>{isOpen ? <IoIosArrowUp className='w-4 h-4' /> : <IoIosArrowDown className='w-4 h-4' />}</div>}
                    </>
                  )}
                </Link>
                  ...
                  <div className='ml-8 space-y-1 animate-fadeIn'>
                    {subMenu.map(({ label: subLabel, to: subTo, icon: subIcon }) => (
                      <Link
                        ...
                        title={subLabel}
                      >
                        <div className='flex-shrink-0 text-slate-400 group-hover:text-emerald-600 transition-colors duration-200'>{subIcon}</div>
